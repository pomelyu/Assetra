...
- 共通：最上方為可選交易名稱（最多 30 字元），其後為類型、交易日期與時間（台北時間，24 小時制 HH:mm）、相關帳戶與可選備註；幣別由帳戶決定。
...
- 比例運算不先捨入；沖銷成本四捨五入至幣別最小單位。全部賣出時完整沖銷剩餘成本，吸收尾差。
- 利息全額增加投資帳戶已實現損益及一般帳戶成本、現值，不改變投資成本或現值。
- 損益調整只改變投資現值與未實現損益，不改變成本或現金流。
- 費用只適用手動買賣，合併手續費與稅為 FEE；其他類型不接受費用值。
- 名稱由 Data API trim；未填或純空白時依類型產生並保存「轉帳」、「收入」、「支出」、「投資買入」、「投資賣出」、「利息」或「損益調整」。已有名稱不隨其他欄位變更，清空後儲存才重新產生。

## Actions, Validation and States

- 新交易的帳戶須未封存、類型與幣別有效；無可選帳戶時顯示「尚未建立資料」。
- 新建時可選適用的交易類型；編輯既有交易時類型唯讀，Data API 亦拒絕任何變更 KIND 的 update。
- 交易名稱超過 30 字元時拒絕儲存並保留表單；不得截斷。
- 手動賣出要求 V > 0、0 < AMOUNT <= V、0 <= FEE <= AMOUNT；一般帳戶可為負餘額。
- 儲存刷新受影響摘要與歷史；取消不保存。編修或刪除必須原子性重算兩端與後續手動交易，無效則整筆回復。
- 既有週快照不隨交易編修改寫。儲存中防止重複提交；失敗保留表單。
...
- 自訂或預設交易名稱可建立、載入與修改；清空後依原類型重產生，既有交易類型不可變更。
