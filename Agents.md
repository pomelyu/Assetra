...
- 交易名稱為最多 30 字元的正式欄位；未填時由 Data API 依交易類型及內容產生並保存。交易類型只在建立交易時選擇，建立後不可變更。
...
- 公開 Data API 的 `Money.units`、`ShareQuantity.units` 與匯率參數皆使用畫面上的實際數值，呼叫端不處理倍率。資料庫內部依程式常數縮放為整數：TWD／JPY 金額為整數、USD／EUR 金額兩位小數、匯率兩位小數、股數四位小數（倍率 10000）。
...
- **版面風格**：iOS 原生 Modern Clean 風格、大圓角白色卡片（Subtle Shadow）、清晰的資訊邊界與充足的留白節奏。考量 RWD 排版
...
- 當開發者回饋 bug，或在開發過程中出現 data 相關的 bug，先檢查此問題是否和 `docs/testing.md` 的測試相關。
  - 項目標示 [x], [ ] 分別表示「已有測試」、「尚未測試」
  - 如果有關，且為 [ ]。完成測試後，將項目標記完成 [x]
